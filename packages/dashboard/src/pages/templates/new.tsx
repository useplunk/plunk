...
interface TemplateValues {
  ...
  type?: "MARKETING" | "TRANSACTIONAL" | undefined;
  style?: "PLUNK" | "HTML" | undefined;
}
    ...
    watch((value, { name, type }) => {
      if (name === "email") {
        if (
          value.email &&
          project?.email &&
          !value.email.endsWith(project.email.split("@")[1])
        ) {
          setError("email", {
            type: "manual",
  ...
  const create = async (data: TemplateValues) => {
    toast.promise(
      network.mock<Template, typeof TemplateSchemas.create>(
        ...
        {
          ...data,
          style: data.style as "PLUNK" | "HTML",
          type: data.type as "MARKETING" | "TRANSACTIONAL",
        },
      ),
      {
      ...
      <Dashboard>
        ...
        >
          <form
            ...
            className="space-y-6 sm:space-y-0 sm:grid sm:gap-6 sm:grid-cols-6"
          >
            <Input
